...
def main():
    ...
    # Split strophes by empty lines
    strophes = [s.strip() for s in content.split("\n\n") if s.strip()]
    print(f"Total Strophes: {len(strophes)}")
    
    # Primes: 2, 3, 5, 7...
    # Indices (0-based): 1, 2, 4, 6...
    # Note: Strophe is conventionally 1-based.
    ...
    for i in [2, 3, 5, 7]:
        if i <= len(strophes):
            s_primes.append(strophes[i-1]) # i-1 for 0-based
    candidates.append(("Primes_2_3_5...", "".join(s_primes)))
    
    # Transposed Primes: 2, 3, 5 (rearranged to 5, 3, 2?)
    s_rev = s_primes[::-1]
    candidates.append(("Primes_Rev", "".join(s_rev)))
    
    # All Strophes Permuted by Primes?
    # No, too complex.
    
    modes = ['C-K', 'K-C', 'C+K']
    
    for name, txt in candidates:
        key_indices = parse_oe_text(txt)
        limit = min(len(cipher), len(key_indices))
        print(f"\n--- Key: {name} (Len {len(key_indices)}) ---")
        ...
        for mode in modes:
            ...
            for i in range(limit):
                ...
                res.append(val)
            ioc = calc_ioc(res)
            txt_prev = "".join([LATIN_TABLE[x] for x in res[:60]])
            print(f"Mode {mode}: IoC={ioc:.4f} | {txt_prev}...")    
